  ...
  agents
    .map((agent, index) => {
      ...
      const headshotImg = headshotUrl
        ? `<div class="agent-headshot-wrapper"><img src="${escapeAttr(headshotUrl)}" alt="${escapeAttr(fullName)}" class="agent-headshot-img"${cropStyle} onerror="this.onerror=null; this.parentElement.style.display='none'; const placeholder = this.parentElement.nextElementSibling; if(placeholder) placeholder.style.display='flex';"></div>`
        : ''
