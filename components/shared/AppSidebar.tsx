...
import {
  ...
  Settings, Menu, PanelLeft, ChevronRight,
...
export default function AppSidebar({ children, logoUrl }: AppSidebarProps) {
  ...
  return (
    <div className="min-h-screen bg-luxury-light">
      ...
      <aside
        className={`
          fixed top-0 left-0 bottom-0 bg-luxury-light border-r border-luxury-gray-5/50 z-50 flex flex-col
          transition-all duration-300 ease-out overflow-hidden
          ${isMobile
            ? (sidebarOpen ? 'w-[220px] translate-x-0' : 'w-[220px] -translate-x-full')
      ...
      >
          ...
          {navItems.map((item) => {
            ...
            if (item.disabled) {
              return (
                <div
                  key={item.href}
                  className={`flex items-center gap-3 px-3 py-2 rounded-md opacity-30 cursor-not-allowed ${!sidebarOpen ? 'justify-center' : ''}`}
                  title={!sidebarOpen ? item.label : undefined}
                >
                  <Icon size={18} className="text-luxury-gray-3 flex-shrink-0" strokeWidth={1.5} />
                  {sidebarOpen && <span className="text-[13px] text-luxury-gray-3">{item.label}</span>}
                </div>
              )
            }
        ...
        <div className="px-3 py-2 flex-shrink-0">
          <button
            className={`flex items-center gap-3 px-3 py-2 rounded-md text-luxury-gray-3 hover:bg-luxury-gray-5/40 hover:text-luxury-gray-1 transition-colors w-full ${!sidebarOpen ? 'justify-center' : ''}`}
          >
            <Search size={18} className="flex-shrink-0" strokeWidth={1.5} />
            {sidebarOpen && <span className="text-[13px] font-medium">Search</span>}
          </button>
        </div>
        ...
        <div className="flex-shrink-0 border-t border-luxury-gray-5/50 relative">
          <div className="p-3">
            {sidebarOpen ? (
              ...
              <button
                onClick={() => setUserMenuOpen(!userMenuOpen)}
                className="w-8 h-8 rounded-full bg-luxury-accent flex items-center justify-center text-white text-xs font-semibold mx-auto hover:opacity-80 transition-opacity"
              >
                {user.preferred_first_name?.[0]}{user.preferred_last_name?.[0]}
              </button>
            )}
