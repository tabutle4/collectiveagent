...
/**
 * Production charts - the date-range header and the four donuts.
 *
 * Extracted from the ops dashboard so the BROKER dashboard can show the same
 * charts without a second copy of the metric math. The maths here decides
 * volume, units, agent net and office net for the whole company; two versions
 * of it would drift, and a chart that disagrees with the ops chart about the
 * same quarter is worse than no chart.
 *
...
export default function ProductionCharts({
  ...
  if (dateStr > today) return
    ...
    return {
      metrics: { volume, units, agentNet, officeNet },
      volumeByType: volByType,
      unitsByTeam: unitTeam,
      unitsByOffice: unitOffice,
      agentNetByTeam: agentTeam,
      agentNetByOffice: agentOffice,
