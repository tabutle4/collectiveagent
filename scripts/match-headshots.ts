...
function fuzzyMatch(extracted: string, userFullName: string): boolean {
  ...
  if (userNoApostrophe.includes(extractedNoApostrophe)) {
    ...
    if (extractedFirst === userFirst || userFirst.startsWith(extractedFirst) || extractedFirst.startsWith(userFirst)) {
  ...
  if (extractedNoApostrophe.includes(userNoApostrophe)) {
    if (extractedFirst === userFirst || extractedFirst.startsWith(userFirst) || userFirst.startsWith(extractedFirst)) {
  ...
  if (userFirstFull.startsWith(extractedFirstFull) || extractedFirstFull.startsWith(userFirstFull)) {
    if (extractedLast === userLast) {
      return true
    }
  }
...
async function matchHeadshots() {
  ...
  const { data: users, error: usersError } = await supabase
    .from('users')
    .select('id, first_name, last_name, preferred_first_name, preferred_last_name, email, headshot_url')
    .eq('is_active', true)
  ...
  for (const filename of files) {
    ...
    const baseName = extractedName.toLowerCase().replace(/\s*\([0-9]+\)\s*$/, '').trim()
    ...
    if (matched && matchedUser) {
      ...
      if (updateError) {
        ...
        const normalizedExtracted = normalizeName(extractedName)
        const isFuzzy = normalizedExtracted !== normalizeName(`${matchedUser.preferred_first_name} ${matchedUser.preferred_last_name}`) && 
                       normalizedExtracted !== normalizeName(`${matchedUser.first_name} ${matchedUser.last_name}`)
        const fuzzyLabel = isFuzzy ? ' (fuzzy match)' : ''
        console.log(`✅ Matched: ${filename} → ${matchedUser.preferred_first_name} ${matchedUser.preferred_last_name} (${matchTypeLabel}${fuzzyLabel})`)
      }
    } else {
      results.push({
        filename,
