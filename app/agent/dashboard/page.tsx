  ...
  return (
    <div className="overflow-x-hidden">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        <button onClick={() => router.push('/transactions/new')} className="btn btn-primary flex items-center gap-1.5">
      ...
      <div className="container-card mt-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => router.push('/transactions')} className="text-xs text-luxury-accent hover:text-luxury-gray-1 transition-colors">View All</button>
        </div>

        {recentTransactions.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-sm text-luxury-gray-3 mb-4">No transactions yet</p>
            <button onClick={() => router.push('/transactions/new')} className="btn btn-primary">Create Your First Transaction</button>
          </div>
        ) : (
          <>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {recentTransactions.map((t) => (
                    <tr key={t.id} className="tr-luxury-clickable" onClick={() => router.push(`/transactions/${t.id}`)}>
            ...
            <div className="md:hidden space-y-3">
              {recentTransactions.map((t) => (
                <div key={t.id} className="inner-card cursor-pointer" onClick={() => router.push(`/transactions/${t.id}`)}>
