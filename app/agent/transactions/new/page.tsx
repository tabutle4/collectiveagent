...
import { ChevronLeft, ChevronRight, Save, FileText, Upload, Check } from 'lucide-react'
import { ProcessingFeeType } from '@/lib/transactions/types'
import { getVisibleSlides, REPRESENTATION_TYPES, TENANT_TYPES, LOAN_TYPES, LEAD_SOURCES, FLYER_DIVISIONS } from '@/lib/transactions/constants'
import SlideNav from '@/components/transactions/SlideNav'
...
export default function CreateTransactionPage() {
  ...
  const currentSlideConfig = visibleSlides[currentSlide]
  const isFirstSlide = currentSlide === 0
  const isLastSlide = currentSlide === visibleSlides.length - 1
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={handleSaveDraft} disabled={saving || !form.processing_fee_type_id} className="btn btn-secondary flex items-center gap-1.5 disabled:opacity-50">
            <Save size={14} /> {saving ? 'Saving...' : 'Save Draft'}
          </button>
          <button onClick={() => setShowSummary(!showSummary)} className="btn btn-secondary lg:hidden flex items-center gap-1.5">
            <FileText size={14} /> Summary
          </button>
        </div>
      </div>

      <ProgressBar current={currentSlide + 1} total={visibleSlides.length} label={currentSlideConfig?.title} />

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        {/* Slide Navigation */}
        <div className="hidden lg:block lg:col-span-2">
          <SlideNav slides={visibleSlides} currentSlide={currentSlide} onNavigate={setCurrentSlide} />
        </div>

        {/* Main slide area */}
        <div className={`${showSummary ? 'hidden lg:block' : ''} lg:col-span-7`}>
          <div className="container-card min-h-[500px] flex flex-col">
            <div className="flex-1">
              {currentSlideConfig?.id === 'type' && <SlideType types={transactionTypes} selected={form.processing_fee_type_id} onSelect={selectTransactionType} />}
              {currentSlideConfig?.id === 'property' && <SlideProperty form={form} onChange={updateForm} />}
              {currentSlideConfig?.id === 'client' && <SlideClient form={form} onChange={updateForm} />}
              {currentSlideConfig?.id === 'financials' && <SlideFinancials form={form} onChange={updateForm} />}
              {currentSlideConfig?.id === 'dates' && <SlideDates form={form} onChange={updateForm} />}
              {currentSlideConfig?.id === 'details' && <SlideDetails form={form} onChange={updateForm} />}
              {currentSlideConfig?.id === 'title_info' && <SlideTitleInfo form={form} onChange={updateForm} />}
              {currentSlideConfig?.id === 'documents' && <SlideDocuments requiredDocs={requiredDocs} />}
              {currentSlideConfig?.id === 'review' && <SlideReview form={form} selectedType={selectedType} />}
            </div>

            {/* Navigation buttons */}
            <div className="flex items-center justify-between pt-5 mt-5 border-t border-luxury-gray-5/50">
              <button onClick={() => setCurrentSlide(prev => Math.max(0, prev - 1))} disabled={isFirstSlide} className="btn btn-secondary flex items-center gap-1.5 disabled:opacity-30">
              ...
              {isLastSlide ? (
                ...
                <button onClick={() => setCurrentSlide(prev => Math.min(visibleSlides.length - 1, prev + 1))} disabled={currentSlideConfig?.id === 'type' && !form.processing_fee_type_id} className="btn btn-primary flex items-center gap-1.5 disabled:opacity-50">
        ...
        {/* Summary panel */}
        <div className={`${showSummary ? 'block' : 'hidden'} lg:block lg:col-span-3`}>
          <SummaryPanel
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        ...
        <div className="inner-card">
          <label className="checkbox-label"><input type="checkbox" checked={form.has_btsa} onChange={(e) => onChange('has_btsa', e.target.checked)} className="w-4 h-4" /><span className="font-semibold">BTSA (Broker to Seller Assigned)</span></label>
          {form.has_btsa && (
            <div className="mt-3"><label className="field-label">BTSA Amount ($)</label><input className="input-luxury" type="number" step="0.01" value={form.btsa_amount} onChange={(e) => onChange('btsa_amount', e.target.value)} /></div>
          )}
        </div>

        <div className="inner-card">
          <label className="checkbox-label"><input type="checkbox" checked={form.has_ecommission} onChange={(e) => onChange('has_ecommission', e.target.checked)} className="w-4 h-4" /><span className="font-semibold">eCommission Advance</span></label>
          {form.has_ecommission && (
            <div className="mt-3"><label className="field-label">eCommission Amount ($)</label><input className="input-luxury" type="number" step="0.01" value={form.ecommission_amount} onChange={(e) => onChange('ecommission_amount', e.target.value)} /></div>
          )}
        </div>

        <div className="inner-card">
          <label className="checkbox-label"><input type="checkbox" checked={form.expedite_requested} onChange={(e) => onChange('expedite_requested', e.target.checked)} className="w-4 h-4" /><span className="font-semibold">Request Expedite ($95)</span></label>
        </div>
      </div>
    </div>
  )
...
function SlideReview({ form, selectedType }: { form: TransactionForm; selectedType?: ProcessingFeeType }) {
  return (
    <div>
      <h2 className="section-title">Review Your Transaction</h2>
      <div className="space-y-4">
        <div className="inner-card">
