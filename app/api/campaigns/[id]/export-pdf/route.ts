...
export async function GET(
  ...
  try {
    ...
    const colors = {
      ...
      gold: rgb(0.788, 0.663, 0.38), // #C9A961
    ...
    if (agents.length > 0) {
      ...
      agents.forEach((agent: any, index: number) => {
        ...
        const commissionPlan = (response?.commission_plan_2026 || '-')
          .replace(/_/g, ' ')
    ...
    if (surveys.length > 0) {
      ...
      const avgRating =
        surveys.reduce((sum: number, s: any) => sum + (s.support_rating || 0), 0) / surveys.length
      if (avgRating > 0) {
        currentPage.drawRectangle({
          x: margin,
          y: y - 45,
      ...
      const independentCount = surveys.filter(
        (s: any) => s.work_preference === 'independent'
      ).length
