...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from('transactions')
      .select(
        'id, property_address, transaction_type, mls_link, listing_date, lead_source, status, dotloop_file_created, listing_input_requested, photography_requested'
      )
    ...
    const filteredTransactions = (transactions || [])
      .filter((t: any) => t.status !== 'closed' && t.status !== 'cancelled')
      .slice(0, 20)
