  ...
  try {
    ...
    if (isUpdateAuth && body.property_address && finalAgentId) {
      ...
      if (body.client_names || body.client_phone || body.client_email) {
        ...
        // .limit(1) on an array, not .single(). PostgREST errors a .single()
        // that matches zero OR more than one row, the error was destructured
        // away, and the code then fell through to the INSERT - so a deal that
        // already had two rows of this type gained a third on every save.
        // Oldest row wins so the choice is stable; a failed lookup writes
        // nothing at all.
