...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Detect the true media type from the file's magic bytes. Browsers and OneDrive
    // sometimes report the wrong file.type (e.g. a JPEG labeled image/png), which the
    // Claude API rejects with a 400. Sniffing the bytes is authoritative.
