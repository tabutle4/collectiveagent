...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: dbUser } = await supabaseAdmin
      .from('users')
      .select(
        'id, email, first_name, last_name, preferred_first_name, preferred_last_name, role, roles, office, commission_plan, full_nav_access, is_licensed_agent, status, is_active, headshot_url, headshot_crop, qualifying_transaction_count, qualifying_transaction_target, waive_coaching_fee, join_date, division, monthly_fee_paid_through, new_signature_completed_at, bank_connected, dashboard_links'
      )
