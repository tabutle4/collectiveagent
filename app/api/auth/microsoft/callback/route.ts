...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: user, error: userError } = await supabaseAdmin
      .from('users')
      .select('id, email, role, is_active, first_name, last_name, preferred_first_name, preferred_last_name')
    ...
    const redirectTo = userRole === 'admin' || userRole === 'broker' || userRole === 'operations' || userRole === 'tc'
      ? '/admin/dashboard'
      : '/agent/dashboard'
