...
export default function CoordinationDetailPage() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setEditData({
          ...
          service_paid: data.coordination.service_paid,
        ...
        // Clear agent search when loading existing data
        setAgentSearch('')
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Also trigger a refresh of the listings table data
        // The form responses page will fetch fresh data when modal is opened
      } else {
  ...
  const handleSendWelcomeEmail = async () => {
    if (!confirm('Send welcome email to the seller?')) {
      return
    }
  ...
  const handleSendWeeklyEmail = async () => {
    if (!confirm('Send weekly report email to the seller?')) {
      return
    }
      ...
      <div className="">
        <div className="container-card mb-6">
          <div className="flex items-center justify-between mb-4">
            <button
              onClick={() => router.push('/admin/coordination')}
              className="text-sm text-luxury-gray-2 hover:text-luxury-black"
            >
              ← Back to Dashboard
            </button>
                  ...
                  {new Date(coordination.payment_due_date) < new Date() ? (
                    <div className="bg-red-50 border border-red-200 p-3 rounded">
                      ...
                      <p className="text-xs text-red-700 mt-1">
                        60-day deadline has passed. Fee should be deducted from agent commission.
                      </p>
                    </div>
                  ) : (
                    <div className="bg-yellow-50 border border-yellow-200 p-3 rounded">
                      ...
                      <p className="text-xs text-yellow-700 mt-1">
                        Due within 60 days or at closing, whichever happens first.
                      </p>
                    </div>
                  )}
                </>
                ...
                <div className="bg-green-50 border border-green-200 p-3 rounded">
                  ...
                  {coordination.payment_date && (
                    <p className="text-xs text-green-700 mt-1">
                      Paid on: {formatDate(coordination.payment_date)}
                    </p>
                  )}
                </div>
              )}
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={saving}
                  className="px-4 py-2 text-sm rounded transition-colors btn-primary disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
          ...
          <div className="container-card mb-6">
            ...
            <div className="bg-luxury-light p-4 rounded">
              ...
              <div className="flex items-center space-x-2">
                <input
                  type="text"
                  value={`${process.env.NEXT_PUBLIC_BASE_URL || 'https://agent.collectiverealtyco.com'}/seller/${coordination.seller_magic_link}`}
                  readOnly
                  className="input-luxury flex-1"
                />
                <button
        ...
        <div className="container-card">
          ...
          <h2 className="text-lg font-medium mb-4 mt-6">Other Actions</h2>
