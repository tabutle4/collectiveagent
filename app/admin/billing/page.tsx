...
export default function AdminBillingPage() {
  ...
  // Payload invoice repair: dry-run result, then the executed result.
  const [offsetScan, setOffsetScan] = useState<any>(null)
  ...
  // Payload invoice repair. execute=false is the dry run (GET, reads only);
  ...
  const runOffsetScan = async (execute: boolean) => {
    if (execute) {
      if (
        !confirm(
          `Clean up ${offsetScan?.invoices_affected} invoice(s) in Payload?\n\nThis removes ${offsetScan?.line_items_to_delete} leftover offset line items. Every removal is a matching pair of one charge and one offset of the same amount, so no invoice balance changes. This cannot be undone in Payload.`
        )
      )
        return
    }
    setOffsetScanning(true)
    setOffsetScanError(null)
    try {
      const res = await fetch('/api/admin/payload/cleanup-offset-duplicates', {
        method: execute ? 'POST' : 'GET',
        cache: 'no-store',
      })
      const data = await res.json()
      ...
      setOffsetScan(data)
      if (execute) {
        // Balances did not move, but Payment History and the per-item
        // breakdown are both derived from the line items, so the cached
        // per-agent data is stale. Clear it, then refetch the one panel that
        // is open - nothing else reloads it, and leaving it cleared renders an
        // empty box.
        setAgentData({})
        if (expandedAgent) await refreshAgentData(expandedAgent)
      }
    } catch (err: any) {
      setOffsetScanError(err.message || 'Scan failed')
    } finally {
      setOffsetScanning(false)
    }
  }
  ...
  return (
    <div>
      ...
      {/* One-time repair for Payload invoices left inflated by the old unstage
          ...
          positive charge instead of removing the offset. Scan first, then
          clean.
          Gated on the permission only - it does NOT hide itself once the
          repair is done, because nothing persists whether a clean scan has
          ever run. Collapsed by default so it stays out of the way. */}
      {userPermissions.includes('can_manage_agent_billing') && (
        <div className="container-card mb-4">
          <button
            ...
            className="text-xs font-semibold text-luxury-gray-2 flex items-center gap-1"
          >
            {showRepair ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
            Payload Invoice Repair
          </button>
          {showRepair && (
          <div className="flex items-start justify-between gap-4 flex-wrap mt-2">
            <div className="min-w-0 flex-1">
              <p className="text-xs text-luxury-gray-3">
                Finds invoices whose charges were doubled by staging and unstaging a debt,
                and removes the leftover offset lines. Balances are never changed, only the
                duplicated charges.
              </p>
              {offsetScan && (
                <div className="mt-2 space-y-0.5">
                  <p className="text-xs text-luxury-gray-1">
                    Scanned {offsetScan.agents_scanned} of {offsetScan.agents_total} agents,{' '}
                    {offsetScan.invoices_scanned} invoices.
                  </p>
                  {offsetScan.invoices_affected === 0 ? (
                    <p className="text-xs text-green-600 font-medium">
                      Nothing to fix. No duplicated invoices found.
                    </p>
                  ) : (
                    <>
                      <p className="text-xs text-amber-800 font-medium">
                        {offsetScan.invoices_affected} invoice
                        {offsetScan.invoices_affected === 1 ? '' : 's'} inflated by{' '}
                        {formatCurrency(offsetScan.total_inflation)} across{' '}
                        {offsetScan.line_items_to_delete} line items.
                      </p>
                      {offsetScan.needs_manual_review > 0 && (
                        <p className="text-xs text-amber-800">
                          {offsetScan.needs_manual_review} of those has a reversal that does not
                          pair to an offset of the same amount. Those are left alone for you to
                          look at in Payload.
                        </p>
                      )}
                      <div className="mt-1 space-y-0.5">
                        {offsetScan.findings.slice(0, 12).map((f: any) => (
                          <p key={f.invoice_id} className="text-xs text-luxury-gray-3 break-words">
                            {f.agent_name}: {f.description} shows{' '}
                            {formatCurrency(f.gross_charges_before)}, should be{' '}
                            {formatCurrency(f.real_charges)}
                          </p>
                        ))}
                        {offsetScan.findings.length > 12 && (
                          <p className="text-xs text-luxury-gray-3">
                            and {offsetScan.findings.length - 12} more
                          </p>
                        )}
                      </div>
                    </>
                  )}
                  {offsetScan.mode === 'executed' && (
                    <p className="text-xs text-luxury-gray-1 font-medium mt-1">
                      Cleanup finished.{' '}
                      {offsetScan.findings.reduce((s: number, f: any) => s + (f.deleted || 0), 0)}{' '}
                      line items removed.
                      {offsetScan.drifted > 0
                        ? ` ${offsetScan.drifted} invoice balance changed unexpectedly, check those in Payload.`
                        : ' No invoice balance changed.'}
                    </p>
                  )}
                  {offsetScan.stopped_early && (
                    <p className="text-xs text-amber-800">{offsetScan.stopped_early}</p>
                  )}
                  {offsetScan.incomplete_agents?.length > 0 && (
                    <p className="text-xs text-amber-800 break-words">
                      Could not fully read the invoices for{' '}
                      {offsetScan.incomplete_agents.join(', ')}. Their totals are
                      incomplete, so treat this run as unfinished for them. An agent billed
                      on a different Payload account will show here every time.
                    </p>
                  )}
                </div>
              )}
              {offsetScanError && (
                <p className="text-xs text-red-600 mt-1">{offsetScanError}</p>
              )}
            </div>
            <div className="flex flex-wrap gap-1.5 flex-shrink-0">
              <button
                onClick={() => runOffsetScan(false)}
                disabled={offsetScanning}
                className="btn btn-secondary text-xs flex items-center gap-1.5 disabled:opacity-50"
              >
                <Search size={12} />
                {offsetScanning ? 'Scanning...' : 'Scan Invoices'}
              </button>
              {offsetScan && offsetScan.mode === 'dry_run' && offsetScan.pairs_to_delete > 0 && (
                <button
                  onClick={() => runOffsetScan(true)}
                  disabled={offsetScanning}
                  className="btn btn-primary text-xs flex items-center gap-1.5 disabled:opacity-50"
                >
                  <CheckCircle size={12} />
                  {offsetScanning
                    ? 'Cleaning...'
                    : `Clean Up ${offsetScan.invoices_affected} Invoice${offsetScan.invoices_affected === 1 ? '' : 's'}`}
                </button>
              )}
            </div>
          </div>
                              ...
                              <div className="space-y-2">
                                {data.invoices.map((inv: any) => (
                                  <div key={inv.id} className="inner-card">
                                    <div className="flex items-start justify-between">
                                      {/* min-w-0 lets the description wrap to
                                          as many lines as it needs instead of
                                          being squeezed by the button column,
                                          which is flex-shrink-0. */}
                                      <div className="min-w-0 flex-1">
                                        ...
                                        {realChargeItems(inv).length > 1 && (
                                          <div className="mt-1 space-y-0.5">
                                            {realChargeItems(inv).map((i: any, idx: number) => (
                                              <p
                                                key={i.id || idx}
                                                className="text-xs text-luxury-gray-3 break-words"
                                              >
                                                {i.description || i.type}: {formatCurrency(i.amount)}
                                              </p>
                                            ))}
                                          </div>
                                        )}
                            ...
                            {data.receipts.length === 0 ? (
                              ...
                              <div className="space-y-1.5">
                                {data.receipts.map((r: any) => (
                                  ...
                                  >
                                    <div className="min-w-0 flex-1">
                                      <p className="text-xs font-medium text-luxury-gray-1 break-words whitespace-pre-wrap">
                                        {r.description}
                                      </p>
                                      <p className="text-xs text-luxury-gray-3 break-words">
                                        {formatDate(r.paid_at)}
                                        {r.method ? ` · ${r.method}` : ''}
                                        {/* Only present for invoices settled
                                            after the app started recording the
                                            actor. Older rows keep the plain
                                            method label. */}
                                        {r.settled_by_name ? ` · Marked paid by ${r.settled_by_name}` : ''}
                                      </p>
                                      {r.settled_note && (
                                        <p className="text-xs text-luxury-gray-3 break-words">
                                          {r.settled_note}
