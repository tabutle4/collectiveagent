...
function PayoutTableRow({ row }: { row: PayoutRow }) {
  ...
  return (
    <tr className="border-b border-luxury-gray-5/30 hover:bg-luxury-gray-6/30">
      <td className="py-2 px-3 text-xs text-luxury-gray-1 font-medium max-w-[180px]">
        {row.transaction_id ? (
          <Link href={`/admin/transactions/${row.transaction_id}`} className="hover:text-luxury-accent flex items-center gap-1">
            {row.address} <ExternalLink size={10} className="opacity-40 flex-shrink-0" />
          </Link>
        ) : row.address}
      </td>
      <td className="py-2 px-3 text-xs text-right text-luxury-gray-2">{row.crc_amount > 0 ? fmt(row.crc_amount) : '—'}</td>
      <td className="py-2 px-3 text-xs text-right text-luxury-gray-2">{a1 ? fmt(a1.amount) : '—'}</td>
      <td className="py-2 px-3 text-xs text-right text-luxury-gray-2">{a2 ? fmt(a2.amount) : '—'}</td>
      <td className="py-2 px-3 text-xs text-right text-luxury-gray-2">{ext ? fmt(ext.amount) : '—'}</td>
      <td className="py-2 px-3 text-xs text-right font-medium text-luxury-gray-1">{agentTotal(row) > 0 ? fmt(agentTotal(row)) : '—'}</td>
      <td className="py-2 px-3 text-xs text-luxury-gray-2 max-w-[120px] truncate">{agentNames(row)}</td>
      <td className="py-2 px-3 text-xs text-luxury-gray-3">{fmtDate(row.cleared_date || row.received_date)}</td>
      <td className={`py-2 px-3 text-xs ${cls}`}>{label}</td>
      <td className="py-2 px-3 text-xs">
        <span className={`font-medium ${row.crc_transferred ? 'text-green-700' : 'text-red-500'}`}>
          {row.crc_transferred ? 'yes' : 'no'}
        </span>
      </td>
      <td className="py-2 px-3 text-xs text-luxury-gray-3 max-w-[120px] truncate">{row.notes || '—'}</td>
    </tr>
  )
}
...
function PayoutsTable({ rows, title, collapsed, onToggle }: {
          ...
          <table className="w-full min-w-[920px]">
            <thead>
              <tr className="border-b border-luxury-gray-5/50">
                {['Address','CRC','Agent 1','Agent 2','Other','Total','Agents','Date','Compliance','Transferred','Notes'].map((h, i) => (
                  <th key={h} className={`pb-2 px-3 text-xs font-semibold text-luxury-gray-3 uppercase tracking-widest ${i > 0 && i < 6 ? 'text-right' : 'text-left'}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map(row => <PayoutTableRow key={row.check_id} row={row} />)
              )}
            </tbody>
            {rows.length > 0 && (
              <tfoot>
...
export default function PayoutsReportPage() {
  ...
  return (
    <div>
      ...
      <PayoutsTable rows={paidRows} title="Paid Most Recently" collapsed={paidCollapsed} onToggle={() => setPaidCollapsed(v => !v)} />
      <PayoutsTable rows={holdRows} title="On Hold" collapsed={holdCollapsed} onToggle={() => setHoldCollapsed(v => !v)} />
